import { Drawer, Text } from '@heroui/react'
...
type SubView = QueueDrawerProps['currentSubView']
...
// ---------------------------------------------------------------------------
// Icon primitives
// Minimal 24 × 24 SVGs - no third-party icon dependency.
// ---------------------------------------------------------------------------
...
type NavLinkProps = {
  ...
  // middle-click / keyboard tab focus all see a genuine navigation target.
...
export function QueueDrawer({
  ...
  // Per spec section 5 / edge case: if username becomes null while user menu
  // is open, the user identity row disappears. Warn that the parent must reset
  // isUserMenuOpen to false when username transitions to null.
  if (username === null && isUserMenuOpen) {
    // Contract reminder: parent must reset isUserMenuOpen when username becomes null.
    ...
    >
      <Drawer.Content placement="left" className="max-w-xs w-72">
        <Drawer.Dialog aria-label="Navigation menu">
          {/* ----------------------------------------------------------------
              Header: brand mark + user identity row
          ---------------------------------------------------------------- */}
          <Drawer.Header className="flex flex-col gap-3 pb-0">
            {/* Brand wordmark */}
            <Text type="h3" weight="bold">
              CrowdTune
            </Text>

            {/* User identity row - hidden (opacity-0, no pointer events) when username is null */}
            <button
              type="button"
          ...
          ---------------------------------------------------------------- */}
          <Drawer.Body className="relative overflow-hidden">
